...
public:
    ...
    LGFX(void)
    {
        {                                      // バス制御の設定を行います。
            ...
            cfg.pin_sclk = GPIO_NUM_12; // SPIのSCLKピン番号を設定
            cfg.pin_mosi = GPIO_NUM_11; // SPIのMOSIピン番号を設定
            cfg.pin_miso = GPIO_NUM_13; // SPIのMISOピン番号を設定 (-1 = disable)
            cfg.pin_dc = GPIO_NUM_9;    // SPIのD/Cピン番号を設定  (-1 = disable)
        ...
        {                                        // 表示パネル制御の設定を行います。
            ...
            cfg.pin_cs = GPIO_NUM_10;   // CSが接続されているピン番号   (-1 = disable)
            cfg.pin_rst = GPIO_NUM_3;   // RSTが接続されているピン番号  (-1 = disable)
